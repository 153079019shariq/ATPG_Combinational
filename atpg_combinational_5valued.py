...
def imply(G, pi, val, implications):
    ...
    print "out_edge_list",out_edge_list
    for out_edge in out_edge_list:
...
def has_fault_propagated(G, po_list):
    for item in G.in_edges(po_list, True):
...
def Objective(G, node1, node2, fvalue): 
    ...
    node1 = D_frontier[0]
    for item in G.in_edges(node1, 'x'):
				print item
        #~ if (item[2]["value"] == 'x'):
...
"nodes returns 2-tuple (node(nbr), data-dict)"
def preprocess(G):
